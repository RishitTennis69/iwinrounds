...
interface SetupProps {
  onStart: (session: DebateSession) => void;
}

const Setup: React.FC<SetupProps> = ({ onStart }) => {
  const [topic, setTopic] = useState('');
  const [affirmativeTeam, setAffirmativeTeam] = useState('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!topic || !affirmativeTeam || !negativeTeam || 
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="text"
                value={affirmativeTeam}
                onChange={(e) => setAffirmativeTeam(e.target.value)}
